...
def SGL_MM(C, g_funcs, up_bounds, cs, regs, max_iters=100, epsilon=1e-6,
           verbose=False, verb_solv=False):
    N = C.shape[0]
    alpha = regs['alpha']
    beta = regs['beta']
    gamma = regs['gamma']
    ...
    if not isinstance(cs, list) and np.isscalar(cs):
        cs = [cs]
    if not isinstance(deltas, list) and np.isscalar(deltas):
        deltas = [deltas]
    ...
    for t in range(max_iters):
        ...
        z = utils.Lstar_op(V@np.diag(lambdas)@V.T-1/beta*K)
        ...
        for i, g_func in enumerate(g_funcs):
            ...
            if expr.curvature == "AFFINE":
                contraints.append((expr - cs[i])**2 <= deltas[i])
            elif expr.curvature == "CONCAVE":
                contraints.append((expr - cs[i]) >= -deltas[i])
            else:
                contraints.append((expr - cs[i]) <= deltas[i])

        up_bounds_obj = 0
        for up_bound in up_bounds:
            up_bounds_obj += gamma*up_bound(lambdas_hat, lambdas[1:], N)


        obj = cp.Minimize(cp.sum(-cp.log(lambdas_hat)) \
            + beta/2*cp.sum_squares(lambdas_hat-d) + up_bounds_obj)

        prob = cp.Problem(obj, contraints)        
        prob.solve(verbose=verb_solv)
        lambdas = np.insert(lambdas_hat.value,0,0)  # Prepend eigenvalue 0 at the begining
        
        if np.all((L == 0)):
            print('WARNING: L_hat is 0. Returning L from previous iteration')
            return prev_L, lambdas
        ...
        if verbose:
            ...
            print('{}. status: {} - L_cnv: {:.6f} - Lam_cnv: {:.3f} - L-VLambdaL\': {:.3f}'.
                format(t, prob.status, L_conv, lambd_conv, sim_Ls))
...
def SGL_show_conv(C, g_funcs, up_bounds, cs, regs, A_true, 
    ...
    for t in range(max_iters):
        ...
        conv_A[t] = np.linalg.norm(As[:,:,t+1]-As[:,:,t], 'fro')**2/np.linalg.norm(As[:,:,t], 'fro')**2
